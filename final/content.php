
    <?php 
      $fName = basename(__FILE__);
      $pageTitle = 'Content';
      include 'header_template.php';
     ?>

    <div class="container">

        <div class="row">
            <div class="box">
                <div class="col-lg-12">
                    <hr>
                    <h2 class="intro-text text-center">
                        <strong>Career Prep Materials</strong>
                    </h2>
                    <hr>
                </div>
                <div class="col-lg-12 text-center">
                    <img class="img-responsive img-border img-full" src="img/people.jpeg" alt="people on train">
                    <h2>Networking
                        <br>
                        <small>Get out there and meet people.</small>
                    </h2>
                    <p>You won't find your dream job without making connections. Here are some tips for connecting the right contacts.</p>
                    <a href="#" class="btn btn-default btn-lg">Read More</a>
                    <hr>
                </div>
                <div class="col-lg-12 text-center">
                    <img class="img-responsive img-border img-full" src="img/tools.jpeg" alt="computer and ipad">
                    <h2>Job Hunting
                        <br>
                        <small>Using the right tools to find the right job.</small>
                    </h2>
                    <p>Here are a list of job search resources.</p>
                    <a href="#" class="btn btn-default btn-lg">Read More</a>
                    <hr>
                </div>
                <div class="col-lg-12 text-center">
                    <img class="img-responsive img-border img-full" src="img/confidence.jpeg" alt="person on mountain">
                    <h2>Interviewing
                        <br>
                        <small>Get prepared, get confident.</small>
                    </h2>
                    <p>Now that you've found the right job, you need to land it. Getting the interview is only the first step. Here are some tips to help you interview with confidence.</p>
                    <a href="#" class="btn btn-default btn-lg">Read More</a>
                    <hr>
                </div>
            </div>
        </div>

    </div>
    <!-- /.container -->

    <?php  
        include 'footer_template.php'
    ?>
